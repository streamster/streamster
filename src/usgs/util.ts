...
*/
export const validateRequiredParameters = (queryParameters: queryParameters): void => {
 ...
 */
export const prepareUrl = (service: Services, queryParameters: queryParameters): string => {
 ...
 * @returns {array}
 */
export const formatGroupedTimeSeriesData = (data: any, options: usgsFetchOptions) => {
  const { collapseMethods } = options;
  const { timeSeries } = data.value;

  if (collapseMethods) {
    const parameterSeries = [...new Set(timeSeries.map((t: any) => t.variable.variableDescription))];
    const locationSeries = [...new Set(timeSeries.map((t: any) => t.sourceInfo.siteName))];
...
export const updateQueryParameters = (existingParams: queryParameters, newParams: queryParameters): queryParameters => {
  return {
    ...existingParams,
    ...newParams,
  }
